...
let db = window.openDatabase('data', '1.0', 'data', 20000);
...
function confirmTrip(e) {
    ...
    $('#page-create #frm-confirm #place').text($('#page-create #frm-register #place').val());
...
function registerTrip(e) {
    ...
    let place = $('#page-create #frm-register #place').val();

    db.transaction(function (tx) {
        let query = `INSERT INTO Trip (Name, Destination, Description, Risk, Vehicle, Hotel, Place, Date) VALUES (?, ?, ?, ?, ?, ?, ?, julianday('${date}'))`;
        tx.executeSql(query, [name, destination, description, risk, vehicle, hotel, place], transactionSuccess, transactionError);
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            if (result.rows[0] != null) {
                ...
                $('#page-detail #detail #place').text(result.rows[0].Place);
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            if (result.rows[0] != null) {
                ...
                $(`#page-detail #frm-update #vehicle`).val(result.rows[0].Vehicle);
...
function updateTrip(e) {
    ...
    db.transaction(function (tx) {
        let query = `UPDATE Trip SET Name = ?, Destination = ?, Description = ?, Vehicle= ?, Risk = ?, Date = julianday('${date}') WHERE Id = ?`;

        tx.executeSql(query, [name, destination, description, vehicle, risk, id], transactionSuccess, transactionError);
